...
function MoverRow({ item }) {
  ...
  return (
    ...
    >
      <span className="font-mono text-sm font-bold text-gray-200 w-14 shrink-0">
        {item.kode_saham}
      </span>
      <span className="font-mono text-sm text-gray-400 flex-1 text-right tabular-nums">
        {item.harga_terakhir != null
          ? item.harga_terakhir.toLocaleString('id-ID', { maximumFractionDigits: 0 })
          : '—'}
      </span>
      <span className={`font-mono text-sm font-semibold tabular-nums w-16 text-right shrink-0
...
export default function TopMovers({ items }) {
  ...
  const Section = ({ label, icon, color, data }) => (
    <div className="rounded-xl border border-gray-800/50 bg-[#13151f] p-4">
      ...
      <div className="space-y-1">
        {data.length > 0
          ...
          : <p className="text-xs text-gray-700 px-2.5 py-2">Belum ada data</p>
        }
      </div>
    </div>
  )
